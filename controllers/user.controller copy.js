...
exports.Login = {
    ...
    getToken: async (req, res) => {
        ...
        try {
            // kakaoAccessToken으로 사용자 정보 요청하기
            kakaoUserInfo = await axios({
                method: "GET",
            ...
            if (kakaoUserInfo) {
                ...
                const { session, query } = req;
                //계정 연동
                const isLinkedUserDB = await func_link.Kakao.linkUser(session, "kakao", authData);

                //로그인 기록 존재할 경우
                if (isLinkedUserDB) {
                    const email = authData.kakao_account.email;
                    const isExistedUser = await UserService.SelectExistedUser(email);
                    ...
                    if (isExistedUser == 1) {
                        ...
                        if (store_refresh_token != -1) {
                            ...
                            // let data = user_id + access_token;
                            // res.send({ 'user_id': user_id, 'access_token': access_token, redirect_path: "/user/main" });
                        ...
                        const insertUser = await UserService.insertUser(user_id, email, provider, jwt_token);
