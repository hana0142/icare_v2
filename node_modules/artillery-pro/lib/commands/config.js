 ...
 *
 */
"use strict";const util=require("../util"),awsUtil=require("../aws-util"),{ALLOWED_CONFIG_PARAMETERS}=require("../constants");async function set(options){const name=options.name;var value=options.value;options.region||(console.log(util.formatError(new Error("A region must be specified, e.g. us-east-1"))),process.exit(1)),-1===util.supportedRegions.indexOf(options.region)&&(console.log(`Unsupported region (${options.region}) provided. Please specify one of: ${util.supportedRegions.join(", ")} `),process.exit(1)),name&&value||(console.log(util.formatError(new Error("Both name and value must be provided"))),process.exit(1));var pname=name.toUpperCase();ALLOWED_CONFIG_PARAMETERS.indexOf(pname)<0&&(console.log(util.formatError(new Error(`Parameter name must be one of: ${ALLOWED_CONFIG_PARAMETERS.join(", ")}`))),process.exit(1));try{var type="NPM_TOKEN"===pname?"SecureString":"String";await awsUtil.putParameter(`/artilleryio/${pname}`,value,type,options.region),console.log(`Parameter ${pname} in ${options.region} has been updated`,"SecureString"==type?"(encrypted)":"")}catch(err){console.log(util.formatError(err)),process.exit(1)}}module.exports={set:set};